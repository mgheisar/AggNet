...
import torchvision
import torch
from DSH import HashSetNet, get_clusters, LogisticReg, acc_authentication
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alpha', '--alpha', type=float, default=alpha,
                        help='Regularization for DSH loss (default: "alpha")')
    ...
    if start:
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target, img_file, class_id) in enumerate(train_loader):
                ...
                loss1, accuracy, accuracy_bin = loss_fn(v_set, v_f, m_set, logisticReg)
                v = torch.cat([v_set, v_f], dim=0)
                loss2 = (v.abs() - 1).abs().sum(dim=1).mean()
