...
def acc_authentication(model, H0_loader, target, n_classes, v_set, v_f, m_set, n_batch_verif):
    H0_id, H0_data = [], []
    if target is None:
        # ----- Validation(since H0 is selected from training set)
        for batch_idx, (data, labels, img_file, class_id) in enumerate(H0_loader):
            if batch_idx >= (n_batch_verif/2):
                break
            H0_id.append(labels)
            H0_data.append(data)
        ...
        try:
            ...
            for batch_idx, (data, labels) in enumerate(H0_loader):
                H0_id.append(labels)
                H0_data.append(data)
        H0_id = torch.stack(H0_id).flatten(start_dim=0, end_dim=1)
        H0_data = torch.stack(H0_data).flatten(start_dim=0, end_dim=1)
